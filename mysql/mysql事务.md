...
<p><b>什么为间隙锁？</b><br>id为(4,8),(8,18),(18,正无穷)，这三个区间，在事务1中执行
UPDATE user SET name = "jerrys" WHERE id > 4 AND id < 15;  其它事务没法在这个范围所包含的所有记录以及行
记录所在间隙里插入或修改任何数据，即id在(4,18]区间都无法修改数据。<b>间隙锁是在可重复读隔离级别才会生效</b></p>


<h3>无索引行锁会升级为表锁</h3>
<p>锁主要是加在索引上，如果对非索引字段更新，行锁可能会变为表锁。<b>InnoDB的行锁是针对索引加的锁，
不是针对记录加的锁。并且该索引不能失效，否则都会从行锁变为表锁。</b></p>
